...
import { acceptBet, createBet, getSpecificBetOnChain } from "./logic/chain-calls";
...
const AcceptBet = () => {
    ...
    const onAccept = async() => {
        await acceptBet(wallet, new anchor.web3.PublicKey(fire.Address), new anchor.web3.PublicKey(address), new anchor.web3.PublicKey("4ke5umoZLkRByReUXZFSUeYtD6y6WQze6cbwH9nEPefE"));
    }
    ...
    return (
        <div className='flex flex-col w-[calc(100vw-5rem)] min-h-[calc(100vh-6rem)] float-right justify-center items-center bg-dark-bg py-4'>
            ...
            <div className="flex flex-col justify-between items-center w-6/12 gap-5 bg-white p-10 rounded-md">
                ...
                <div className="flex"> 
                    <button className="bg-gray-500 p-1 rounded-sm text-white" onClick={onAccept}>Accept Bet</button>
                </div>
                </div>
