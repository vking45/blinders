...
import { getSpecificBetOnChain, withdrawBet } from "./logic/chain-calls";
...
const WithdrawBet = () => {
    ...
    const onWithdraw = async() => {
        await withdrawBet( wallet, new anchor.web3.PublicKey(fire.Address), new anchor.web3.PublicKey(address) ,new anchor.web3.PublicKey("4ke5umoZLkRByReUXZFSUeYtD6y6WQze6cbwH9nEPefE"), new anchor.web3.PublicKey(bet.creator.toBase58()));
    }
    ...
    return (
        <div className='flex flex-col w-[calc(100vw-5rem)] min-h-[calc(100vh-6rem)] float-right justify-center items-center bg-dark-bg py-4'>
            <h3 className="text-3xl text-center text-white my-5"> Withdraw The Bet </h3>
            <div className="flex flex-col justify-between items-center w-6/12 gap-5 bg-white p-10 rounded-md">
                ...
                <div className="flex justify-center items-center">
                    ...
                    <span>{ loaded ? bet.amount.toNumber() / 10**6 : ""}</span>
                </div>

                <div className="flex"> 
                    <button className="bg-gray-500 p-1 rounded-sm text-white" onClick={onWithdraw}>Withdraw Bet</button>
                </div>
                </div>
